...
public final class ModelData implements Serializable {
  ...
  private transient Map< String, Integer > fieldsIndex = new HashMap<  >();
  private transient Map< String, int[] > fieldGroupsIndex = new HashMap<  >();
  private transient List< IModelFieldGroup[] > fieldGroups = new ArrayList<  >();
  ...
  private void indexateFields() {
    ...
    Map<String, Integer> newFieldsIndex = new HashMap<>( fields.length );
    ...
    addFieldGroupsRecursively( upper, 0, new HashMap<  >() );
  }

  private void calculateRowspan( IModelFieldGroup fieldGroups[] ) {
    List< IModelFieldGroup > parents = new ArrayList<  >();
    FieldGroupsLoop:
  ...
  private void addFieldGroupsRecursively( IModelFieldGroup upper[], int level, Map< String, Integer > fieldPosIndex ) {
    ...
    int x = 0;
    int posX = 0;
    List< IModelFieldGroup > current = new ArrayList<  >();
    for ( IModelFieldGroup fieldGroup : upper ) {
      int rowspan = fieldGroup.getRowspan();
  ...
  public Iterable< IModelFieldGroup > getAllFieldGroupsFromBottom( final boolean depthFirst ) {
    return new Iterable< IModelFieldGroup >() {
      private final LinkedList< IModelFieldGroup > list = new LinkedList<  >( Arrays.asList( fields ) );
  ...
  public Iterable< IModelFieldGroup > getAllFieldGroupsFromTop( final boolean depthFirst ) {
    return new Iterable< IModelFieldGroup >() {
      private final LinkedList< IModelFieldGroup > list = new LinkedList<  >( fieldGroups.isEmpty() ? Collections.EMPTY_LIST : Arrays.asList( fieldGroups.get( 0 ) ) );
