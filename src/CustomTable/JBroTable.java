...
public class JBroTable extends JTable {
  ...
  public void reorderFields( String newFields[] ) {
    ...
    LinkedHashSet< Integer > iold = new LinkedHashSet<  >( modelFields.length );
    for ( int i = 0; i < columnModel.getColumnCount(); i++ ) {
      TableColumn column = columnModel.getColumn( i );
      iold.add( column.getModelIndex() );
    }
    // New order of visible columns
    LinkedHashSet< Integer > inew = new LinkedHashSet<  >( modelFields.length );
      for (String newField : newFields) {
          if (newField == null) {
              continue;
          }
          Integer pos = modelPositions.get(newField);
    ...
    for ( Iterator< Integer > it = iold.iterator(); it.hasNext(); ) {
      ...
      if ( !inew.contains( pos ) ) {
        ...
        if ( field.isManageable() ) {
          ...
          ArrayList< Integer > swap = new ArrayList<  >( inew.size() + 1 );
          swap.addAll( inew );
